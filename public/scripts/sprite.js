...
const Sprite = function(ctx, x, y) {
    ...
    const drawSprite_vertical = function () {
        /* Save the settings */
        ctx.save();
        ...
        // ctx.fillStyle = "red";
        // ctx.globalAlpha = 0.6;
        // ctx.fillRect(parseInt(), parseInt(y - size.height / 2),
        //              size.width, size.height);
        // context.clearRect(x - size.width / 2, y - size.height / 2, sequence.width, sequence.height);
        ctx.imageSmoothingEnabled = false;
        ctx.drawImage(
            sheet,
            sequence.x, sequence.y+ index * 64,
    ...
    const drawSprite_horizontal = function () {
        /* Save the settings */
        ctx.save();
    ...
    const draw_horizontal = function() {
        if (isReady()) {
            drawShadow();
            drawSprite_horizontal();
        }
        return this;
    };
    ...
    const update_vertical = function(time) {
        ...
        if (time - lastUpdate >= sequence.timing) {
            ...
            requestAnimationFrame(drawSprite_vertical);
            lastUpdate = 0;
        }
        return this;
    };

    const update_horizontal = function(time) {
        if (lastUpdate == 0) lastUpdate = time;
        if (time - lastUpdate >= sequence.timing) {
            index++;
            if (index >= sequence.count) {
            ...
            requestAnimationFrame(drawSprite_horizontal);
            lastUpdate = 0;
        }
        return this;
    };
    ...
    const clear = function () {
        const XY = getXY();
        const size = getDisplaySize();
        ctx.clearRect(XY.x, XY.y, XY.x + size.width, XY.y + size.height);
    };
